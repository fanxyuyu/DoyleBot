...
#list of all commands: ping, 8ball <question>, clear <qnt>, other names
...
async def clear(ctx, amount: int):
    ...
    await ctx.send(f'Total de mensagens deletadas: {amount}')
    time.sleep(3)
    await ctx.channel.purge(limit = 1)
@clear.error
async def clear_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send('Uso corredo: b.clear <qntd>')
